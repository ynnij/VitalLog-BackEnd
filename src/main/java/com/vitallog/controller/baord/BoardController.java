...
public class BoardController {
	...
	// 전체 게시판 데이터 리턴
	@GetMapping("/api/vitallog/community") 
	public List<Board> getBoardList(){
		return boardService.getBoardList();
	}
	
	// 게시글 작성 
	@PostMapping("/api/vitallog/community")
	public ResponseEntity<?> createBoard(@RequestBody Board board){
		return boardService.createBoard(board);
	}
	
	// 게시글 상세보기
	@GetMapping("/api/vitallog/community/{postid}/detail")
	public Board getBoardDetail(@PathVariable Integer postid) {
		return boardService.getBoardDetail(postid);
	}
	
	//게시글 수정
	@PostMapping("/api/vitallog/community/{postid}")
	...
	//게시글 삭제
	@DeleteMapping("/api/vitallog/community/{postid}")
